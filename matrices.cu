...
__global__ Mat::Mat(int height, int width)
    : m_height{height}
    , m_width{width}
{
    ...
    if (i == 0 && j == 0) {
        auto err = cudaMalloc(&this->m_buffer, height*width*sizeof(float));
        assert(err != NULL);
    }
    this->m_buffer[i][j] = 0;
}

__global__ Mat::Mat(int height, int width, float value)
    : m_height{height}
    , m_width{width}
{
    int i = blockDim.x*blockId.x + threadId.x;
